...
public class BluetoothHandsfree {
    ...
    private class BluetoothPhoneState {
        ...
        private synchronized void handlePreciseCallStateChange(boolean sendUpdate,
            ...
            if (mCall != call) {
                if (call == 1) {
                    // This means that a call has transitioned from NOT ACTIVE
                    // to ACTIVE.
                    // Switch on audio.
                    mCallStartTime = System.currentTimeMillis();
                    audioOn();
        ...
        private int getCdmaCallHeldStatus(CdmaPhoneCallState.PhoneCallState currState,
            ...
            if (currState == CdmaPhoneCallState.PhoneCallState.CONF_CALL) {
                if (prevState == CdmaPhoneCallState.PhoneCallState.THRWAY_ACTIVE) {
                    callheld = 0; // 0: no calls held, as now *both* the caller
                                  // are active
                } else {
                    callheld = 1; // 1: held call and active call, as on
                                  // answering a
                    // Call Waiting, one of the caller *is* put on hold
                }
            } else if (currState == CdmaPhoneCallState.PhoneCallState.THRWAY_ACTIVE) {
                callheld = 1; // 1: held call and active call, as on make a 3
                              // Way Call
                // the first caller *is* put on hold
            } else {
                callheld = 0; // 0: no calls held as this is a SINGLE_ACTIVE
                              // call
            }
            return callheld;
        }
        ...
        private synchronized AtCommandResult toCindResult() {
            ...
            // Hence we ensure that a proper response is sent for the virtual
            // call too.
            ...
            switch (msg.what) {
                case SCO_CLOSED:
                    synchronized (BluetoothHandsfree.this) {
                        ...
                        // TODO finer lock to decouple from other call flow such
                        // as
                        // mWaitingForCallStart change

                        audioOff();
                        // notify mBluetoothPhoneState that the SCO channel has
                        // closed
                ...
                case SCO_CONNECTION_CHECK:
                    synchronized (mBluetoothPhoneState) {
                        ...
                        if (mBluetoothPhoneState.mCall == 1) {
                            // Sometimes, the SCO channel is torn down by HF
                            // with no reason.
                            // Because we are still in active call, reconnect
                            // SCO.
                    ...
                    if (profile == BluetoothProfile.HEADSET) {
                        ...
                        synchronized (BluetoothHandsfree.this) {
                            if (mPendingAudioState) {
                                mBluetoothHeadset.setAudioState(mHeadset.getRemoteDevice(),
                                        mAudioState);
    ...
    /*
     * Put the AT command, company ID, arguments, and device in an Intent and
     * broadcast it.
     */
     ...
     * headset/handsfree, if one is connected. Does not block. Returns false if
     * the user has requested audio off, or if there is some other immediate
     * problem that will prevent BT audio.
     */
    ...
    /**
     * Used to indicate the user requested BT audio on. This will establish SCO
     * (BT audio), even if the user requested it off previously on this call.
     */
    ...
    /**
     * Used to indicate the user requested BT audio off. This will prevent us
     * from establishing BT audio again during this call if audioOn() is called.
     */
    ...
    private AtCommandResult redial() {
        ...
        if (number == null) {
            ...
            // number to redial
            if (VDBG)
                log("Bluetooth redial requested (+BLDN), but no previous " +
    ...
    /**
     * Build the +CLCC result The complexity arises from the fact that we need
     * to maintain the same CLCC index even as a call moves between states.
     */
    private synchronized AtCommandResult gsmGetClccResult() {
        // Collect all known connections
        Connection[] clccConnections = new Connection[GSM_MAX_CONNECTIONS]; // indexed
                                                                            // by
                                                                            // CLCC
                                                                            // index
    ...
    /**
     * Build the +CLCC result for CDMA The complexity arises from the fact that
     * we need to maintain the same CLCC index even as a call moves between
     * states.
     */
    private synchronized AtCommandResult cdmaGetClccResult() {
        ...
        Connection[] clccConnections = new Connection[CDMA_MAX_CONNECTIONS];// indexed
                                                                            // by
                                                                            // CLCC
                                                                            // index
        Call foregroundCall = mCM.getActiveFgCall();
        ...
        // If the Ringing Call state is INCOMING, that means this is the very
        // first call
    ...
    private String cdmaConnectionToClccEntry(int index, Connection c) {
        ...
        if ((prevCdmaCallState == CdmaPhoneCallState.PhoneCallState.THRWAY_ACTIVE)
            ...
            switch (c.getState()) {
                case ACTIVE:
                    // For CDMA since both the connections are set as active by
                    // FW after accepting
                    // a Call waiting or making a 3 way call, we need to set the
                    // state specifically
                    // to ACTIVE/HOLDING based on the mCdmaIsSecondCallActive
                    // flag. This way the
                    // CLCC result will allow BT devices to enable the swap or
                    // merge options
     ...
     * incoming command is <code>AT+FOO=bar,baz</code>, the value of this should
     * be <code>"+FOO"</code>.
        ...
        parser.register("+CKPD", new AtCommandHandler() {
            private AtCommandResult headsetButtonPress() {
                if (mCM.getFirstActiveRingingCall().isRinging()) {
                    ...
                    // If in-band ring tone is supported, SCO connection will
                    // already
    ...
    private void initializeHandsfreeAtParser() {
            ...
            public AtCommandResult handleBasicCommand(String args) {
                if (args.length() > 0) {
                    if (args.charAt(0) == '>') {
                        ...
                        expectCallStart();
                        return new AtCommandResult(AtCommandResult.UNSOLICITED); // send
                                                                                 // nothing
                    }
                }
                return new AtCommandResult(AtCommandResult.ERROR);
                ...
                if (args.length < 4) {
                    ...
                    if (valid) {
                        if ((mRemoteBrsf & BRSF_HF_CW_THREE_WAY_CALLING) == 0x0) {
                            ...
                            sendURC("OK"); // send immediately, then initiate
                                           // audio
                            if (isIncallAudio()) {
                                audioOn();
                            } else if (mCM.getFirstActiveRingingCall().isRinging()) {
        ...
        parser.register("+CLCC", new AtCommandHandler() {
            ...
            public AtCommandResult handleActionCommand() {
                ...
                // Hence we ensure that a proper response is sent for the
                // virtual call too.
                if (isVirtualCallInProgress()) {
        ...
        parser.register("+CHLD", new AtCommandHandler() {
            ...
            public AtCommandResult handleSetCommand(Object[] args) {
                ...
                if (args.length >= 1) {
                    if (args[0].equals(0)) {
                        ...
                        if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                            if (ringingCall.isRinging()) {
                                // Hangup the active call and then answer call
                                // waiting call.
                                if (VDBG)
                                ...
                                // the active call. In CDMA this mean that the
                                // complete
                        ...
                        sendURC("OK");
                        if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                            // For CDMA, the way we switch to a new incoming
                            // call is by
                            // calling PhoneUtils.answerCall().
                            // switchAndHoldActive() won't
                            // properly update the call state within telephony.
                            // If the Phone state is already in CONF_CALL then
                            // we simply send
                            ...
                            if (ringingCall.isRinging()) {
                                ...
                                // Setting the second callers state flag to TRUE
                                // (i.e. active)
                                cdmaSetSecondCallState(true);
                        ...
                        if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                            CdmaPhoneCallState.PhoneCallState state =
                                    PhoneGlobals.getInstance().cdmaPhoneCallState
                                            .getCurrentCallState();
                            // For CDMA, we need to check if the call is in
                            // THRWAY_ACTIVE state
                            if (state == CdmaPhoneCallState.PhoneCallState.THRWAY_ACTIVE) {
                                ...
                                // State is CONF_CALL already and we are getting
                                // a merge call
                                // This can happen when CONF_CALL was entered
                                // from a Call Waiting
            ...
            public AtCommandResult handleTestCommand() {
                ...
                if (isIncallAudio()) {
                    ...
                    // need to update HS with RING when single ringing call
                    // exist
                    mBluetoothPhoneState.ring();
                }
                // already replied
    ...
    /*
     * This class broadcasts vendor-specific commands + arguments to interested
     * receivers.
     */
    ...
    // NOTE: Currently the VirtualCall API does not allow the application to
    // initiate a call
    // transfer. Call transfer may be initiated from the handsfree device and
    // this is handled by
    // the VirtualCall API
    synchronized boolean initiateScoUsingVirtualVoiceCall() {
        ...
        if (mBluetoothPhoneState.sendUpdate() && !isVirtualCallInProgress()) {
            ...
            sendURC(result.toString());
            if (DBG)
                Log.d(TAG, "initiateScoUsingVirtualVoiceCall: Sent Call-setup procedure");
        }
    ...
    /**
     * Debug thread to read debug properties - runs when debug.bt.hfp is true at
     * the time a bluetooth handsfree device is connected. Debug properties are
     * polled and mock updates sent every 1 second
     */
    private class DebugThread extends Thread {
        ...
        /**
         * false to true transition will force an audio (SCO) connection to be
         * established. true to false will force audio to be disconnected
         */
         ...
         * debug.bt.unsol.inband = 1 => AT+BSIR = 0 sent by the AG Other values
         * are ignored.
         */
